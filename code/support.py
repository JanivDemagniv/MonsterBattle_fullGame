...
def import_character(cols,rows,*path):
    ...
    for row, direction in enumerate(('down','left','right','up')):
        ...
        new_dict[direction + f'_idle'] = [frame_dict[0, row]]
    return new_dict

def import_all_character(*path):
    new_dict = {}
    for _,_, image_names in walk(*path):
...
def coast_importer(cols, rows, *path):
    frame_dict = import_tilemap(cols, rows, *path)
    new_dict = {}
    terrians = ['grass','grass_i','sand_i','sand','rock','rock_i','ice','ice_i']
    sides = {
        'topleft': (0,0), 'top': (1,0), 'topright': (2,0),
        'left': (0,1), 'right': (2,1), 'bottomleft': (0,2),
        'bottom': (1,2), 'bottomright': (2,2)
    }
    for index, terrian in enumerate(terrians):
        new_dict[terrian] = {}
        for key, pos  in sides.items():
            new_dict[terrian][key] = [frame_dict[(pos[0] + index * 3, pos[1] + row)] for row in range(0,rows,3)] 
    return new_dict